...
function formatStorageLabel(storage: number, ram?: number): string {
  const storageStr = storage >= 1024 ? `${storage / 1024}TB` : `${storage}GB`;
  return ram ? `${ram}GB/${storageStr}` : storageStr;
}

function buildItemName(item: CartItem): string {
  let name = item.product.name;
  if (item.selectedVariant) {
    name += ` (${formatStorageLabel(item.selectedVariant.storage, item.selectedVariant.ram)})`;
  }
...
function calcItemPrice(item: CartItem): number {
  ...
  const storage = item.selectedVariant ? Number(item.selectedVariant.price) : 0;
  const color   = item.selectedColorPrice ? Number(item.selectedColorPrice) : 0;
...
export default function CartPage() {
  const {
    cart, lang, removeFromCart, updateQuantity,
    cartTotal, clearCart, installmentPlans,
  } = useStore();
  ...
  const activePlans: InstallmentPlan[] = installmentPlans
    .filter((p) => p.isActive)
  ...
  // ── Derived ──
  const selectedPlan: InstallmentPlan | null = selectedPlanId
    ? (activePlans.find((p) => p.id === selectedPlanId) ?? null)
    : (activePlans[0] ?? null);

  const calcCreditTotal = useCallback(
    (interestPercent: number): number =>
      cart.reduce((sum, i) => {
        const price = calcItemPrice(i);
  ...
  useEffect(() => {
    ...
    const [firstName = '', ...rest] = savedName.split(' ');
    const lastName = rest.join(' ');

    let phone = savedPhone.replace(/\D/g, '');
  ...
  // ── GPS ──
  const handleGetLocation = useCallback((): void => {
  ...
  const handleOrder = useCallback(async (): Promise<void> => {
    ...
    try {
      ...
      const interestPercent   = selectedPlan?.interestPercent ?? 0;

      const items: OrderItem[] = cart.map((i) => {
        const price      = calcItemPrice(i);
        const markup     = paymentMethod === 'credit' ? interestPercent : 0;
        const itemPrice  = Math.ceil(price * (1 + markup / 100));
        return {
      ...
      try {
        ...
        if (telegramRes.ok) {
          const data: TelegramOrderResponse = await telegramRes.json();
          telegramSuccess = data.success === true;
        }
        if (!telegramSuccess) {
          console.warn('[Order] Telegram xabar yuborilmadi, davom etiladi.');
        }
      } catch (telegramErr) {
        console.error('[Order] Telegram API xatosi:', telegramErr);
      }
      ...
      try {
        ...
        if (!orderRes.ok || !orderData.success) {
          console.error('[Order] DB saqlashda xato:', orderData.error ?? orderRes.status);
        }
      } catch (dbErr) {
        console.error('[Order] DB ulanishda xato:', dbErr);
      }
      ...
      console.error('[Order] Kutilmagan xato:', err);
      alert(lang === 'uz' ? 'Xatolik yuz berdi. Qaytadan urinib ko\u02BBring.' : 'Произошла ошибка. Попробуйте снова.');
    } finally {
      setSending(false);
    }
  }, [cart, creditTotal, form, lang, paymentMethod, selectedPlan, total, clearCart]);
      ...
      <div className="max-w-lg mx-auto px-4 py-6">
          ...
          <div>
            ...
            <div className="flex gap-2">
              <input
                ...
                onChange={(e: ChangeEvent<HTMLInputElement>) =>
                  setForm((f) => ({ ...f, locationText: e.target.value, locationLat: '', locationLng: '' }))
                }
                    ...
                    return (
                      ...
                      >
                        ...
                        <span className={`text-[10px] font-bold mt-1 px-1.5 py-0.5 rounded ${
                          plan.interestPercent === 0 ? 'bg-green-100 text-green-600' : 'bg-blue-100 text-blue-600'
                        }`}>
                  ...
                  <div className="mt-3 bg-white rounded-lg p-3 border border-yellow-200">
                    ...
                    <div className="flex justify-between text-sm mt-1">
                      ...
                      <span className="font-extrabold text-yellow-600">
                        {formatPrice(monthlyPayment)}/{lang === 'uz' ? 'oy' : 'мес'}
                      </span>
                    </div>
        ...
        <div className="mt-6 bg-gray-50 rounded-xl p-4 space-y-2">
          ...
          {cart.map((item, idx) => {
            ...
            const itemName = buildItemName(item);
            return (
              <div key={idx} className="flex justify-between text-sm text-gray-600">
                <span className="truncate mr-2">{itemName} × {item.quantity}</span>
                <span className="font-semibold flex-shrink-0">{formatPrice(price * item.quantity)}</span>
              </div>
            );
          })}
      ...
      <div className="grid md:grid-cols-5 gap-6">
        ...
        <div className="md:col-span-3 space-y-3">
          {cart.map((item, idx) => {
            ...
            return (
              <div
                key={`${product.id}-${selectedColor}-${selectedVariant?.id ?? idx}`}
                className="bg-white border border-gray-100 rounded-2xl p-4 flex gap-4 items-start shadow-sm"
              >
                ...
                <div className="flex-1 min-w-0">
                    ...
                    <div className="flex flex-wrap gap-1 mt-1">
                      {selectedVariant && (
                        <span className="text-[10px] bg-gray-100 font-semibold px-2 py-0.5 rounded-md text-gray-600">
                          {formatStorageLabel(selectedVariant.storage, selectedVariant.ram)}
                        </span>
                      )}
                      {selectedColor && (
                        <span className="text-[10px] bg-gray-100 font-semibold px-2 py-0.5 rounded-md text-gray-600">
                  ...
                  {activePlans.length > 0 && (
                    <p className="text-[10px] text-gray-500">
                      ...
                      {formatPrice(
                        Math.ceil(
                          (price * (1 + (activePlans[0]?.interestPercent ?? 0) / 100)) /
                          (activePlans[activePlans.length - 1]?.months ?? 12)
                        )
                      )}/{lang === 'uz' ? 'oy' : 'мес'}
                    </p>
                  )}
                ...
                <div className="flex flex-col items-end gap-3 flex-shrink-0">
                  <button
                    onClick={() => removeFromCart(product.id, selectedColor, selectedVariant?.id)}
                  ...
                  <div className="flex items-center gap-2">
                    <button
                      onClick={() => updateQuantity(product.id, quantity - 1, selectedColor, selectedVariant?.id)}
                  ...
                  {activePlans.slice(0, 3).map((plan) => {
                    ...
                    return (
                      <div key={plan.id} className="flex justify-between">
                        <span>
                          {plan.months} {lang === 'uz' ? 'oy' : 'мес'}{' '}
                          {plan.interestPercent > 0 ? `(+${plan.interestPercent}%)` : '(0%)'}
                        </span>
